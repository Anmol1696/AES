	...
	def _shift_rows(self, repetation, inverse = False):
		...
		for i in range(1,4):
			for u in range(i*num):
				for j in range(3):
					(self.node[repetation][j][i],self.node[repetation][j+1][i]) = (self.node[repetation][j+1][i],self.node[repetation][j][i])

	def _mix_columns(self, repetation, inverse = False):
		for j in range(4):
			coltemp = [self.node[repetation][j][i] for i in range(4)]
			if inverse:
